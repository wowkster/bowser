...
use crate::{character_encoding::CharacterEncoding, io_queue::IoQueue};
...
impl HtmlParser {
    ...
    pub fn try_parse(mut self, input_byte_stream: impl Read) -> HtmlParseResult<Document> {
        ...
        println!("Document Character Encoding: {}", self.character_encoding.to_string());
        println!("Document Encoding Confidence: {:?}", self.encoding_confidence);
    ...
    fn determine_encoding<R: Read>(io_queue: &IoQueue<R>) -> (CharacterEncoding, EncodingConfidence) {
        // Step 1: BOM sniffing
        let bytes = (
            io_queue.peek_nth(0),
            io_queue.peek_nth(1),
